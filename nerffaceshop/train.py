...
def init_dataset_kwargs(data, sampling_strategy):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.VideoFolderDataset', path=data, use_labels=True, max_size=None, xflip=False, sampling_strategy=sampling_strategy)
        ...
        raise err # click.ClickException(f'--data: {err}')
...
@click.option('--sampling_strategy', help='Sampling strategy of dataset', metavar='STR', type=click.Choice(['adaptive', 'uniform']), default='adaptive', show_default=True)
...
@click.option('--cmax',         help='Max. feature maps', metavar='INT',                        type=click.IntRange(min=1), default=256, show_default=True)
...
@click.option('--metrics',      help='Quality metrics', metavar='[NAME|A,B,C|none]',            type=parse_comma_separated_list, default='fid10k_10k', show_default=True)
...
@click.option('--deform_type', help='Deformation type for the generator', metavar='STR', type=click.Choice(['instance-specific', 'template+mapping', 'template-only']), required=False, default='template+mapping')
...
@click.option('--g_num_deformable_res', help='Number of deformable layers in generator', metavar='INT', type=click.IntRange(min=0), default=0, required=False, show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=512, d_dim=128, w_dim=512, c_dim=25, l_dim=9, mapping_kwargs=dnnlib.EasyDict(), lmapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict(), lsynthesis_kwargs=dnnlib.EasyDict())
    c.D_kwargs = dnnlib.EasyDict(class_name=None, c_dim=25, l_dim=9, block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(), epilogue_kwargs=dnnlib.EasyDict())
    ...
    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, sampling_strategy=opts.sampling_strategy)
    ...
    c.G_kwargs.mapping_kwargs.num_layers = c.G_kwargs.lmapping_kwargs.num_layers = opts.map_depth
    ...
    c.G_kwargs.class_name = 'training.networks_animation_lit.TriPlaneGenerator'
    c.D_kwargs.class_name = 'training.quad_discriminator_lit.QuadDiscriminator'
    c.G_kwargs.synthesis_kwargs.fused_modconv_default = c.G_kwargs.lsynthesis_kwargs.fused_modconv_default = 'inference_only' # Speed up training by using regular convolutions instead of grouped convolutions.
    ...
    rendering_options = {
        ...
        # Weights for additional regularization
        'lighting_reg':     1e0, 
        'exp_reg':          1e-1, 
        'shape_reg':        1e0, 
    }
    ...
    c.G_kwargs.lsynthesis_kwargs.num_blocks = 2
    c.G_kwargs.synthesis_kwargs.num_fp16_res = c.G_kwargs.lsynthesis_kwargs.num_fp16_blk = 0
    ...
    c.G_kwargs.sr_kwargs = dnnlib.EasyDict(channel_base=opts.cbase, channel_max=opts.cmax, fused_modconv_default='inference_only', sr_num_fp16_res=opts.sr_num_fp16_res)
    ...
    # if opts.deform_type == 'motion-feature':
    #     c.G_kwargs.class_name = 'training.networks_animation_motion_feature.TriPlaneGenerator'
    # elif opts.deform_type == '3D':
    #     c.G_kwargs.class_name = 'training.networks_animation_3D.TriPlaneGenerator'
    # elif opts.deform_type == 'instance-specific' or opts.deform_type == 'residual':
    #     c.G_kwargs.d_dim = 512
    c.G_kwargs.synthesis_kwargs.deform_type = opts.deform_type
    # c.G_kwargs.synthesis_kwargs.spline_grid_size = 8
    c.loss_kwargs.deform_type = opts.deform_type
    c.G_kwargs.synthesis_kwargs.num_deformable_res = opts.g_num_deformable_res
    c.G_kwargs.synthesis_kwargs.num_fp16_res = opts.g_num_fp16_res
    c.G_kwargs.synthesis_kwargs.conv_clamp = 256 if opts.g_num_fp16_res > 0 else None
    c.G_kwargs.lsynthesis_kwargs.conv_clamp = None
    ...
    # Description string.
    desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma:g}-{opts.deform_type}'
