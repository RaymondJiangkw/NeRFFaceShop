...
@register_metric
def fvd10k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fvd = frechet_video_distance.compute_fvd(opts, max_real=None, num_gen=10000)
    return dict(fvd10k_full=fvd)

@register_metric
def fvd5k(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fvd = frechet_video_distance.compute_fvd(opts, max_real=5000, num_gen=5000)
    return dict(fvd5k=fvd)
...
@register_metric
def fid10k_10k_clip(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid_clip(opts, max_real=10000, num_gen=10000)
    return dict(fid10k_10k_clip=fid)
