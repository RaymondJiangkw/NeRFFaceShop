...
class QuadDiscriminator(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        l_dim,                          # Conditioning light (L) dimensionality.
        ...
        if c_dim + l_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim + l_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        ...
        for res in self.block_resolutions:
            ...
            block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=res,
                first_layer_idx=cur_layer_idx, use_fp16=use_fp16, **block_kwargs, **common_kwargs)
            setattr(self, f'v{res}', block)
            cur_layer_idx += block.num_layers
        if (c_dim + l_dim) * 2 > 0:
            self.mapping_v = MappingNetwork(z_dim=0, c_dim=c_dim+c_dim+l_dim+l_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.v4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
        
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
        self.disc_c_noise = disc_c_noise

    def forward(self, frameU, cU, lU, frameV, cV, lV, update_emas=False, **block_kwargs):
        frameU_raw = filtered_resizing(frameU['image_raw'], size=frameU['image'].shape[-1], f=self.resample_filter)
        frameU = torch.cat([frameU['image'], frameU_raw], 1)
        frameV_raw = filtered_resizing(frameV['image_raw'], size=frameV['image'].shape[-1], f=self.resample_filter)
        frameV = torch.cat([frameV['image'], frameV_raw], 1)
        
        # Image Branch.
        img = torch.cat([frameU, frameV], 0)
        c = torch.cat([cU, cV], 0)
        l = torch.cat([lU, lV], 0)
        ...
        cmap = None
        if self.c_dim + self.l_dim > 0:
            if self.disc_c_noise > 0: c += torch.randn_like(c) * c.std(0) * self.disc_c_noise
            cmap = self.mapping(None, torch.cat((c, l), 1))
        x = self.b4(x, img, cmap)
        
        # Video Branch.
        frames = torch.cat([frameU, frameV], 1)
        c = torch.cat([cU, cV], 1)
        l = torch.cat([lU, lV], 1)
        
        xv = None
        for res in self.block_resolutions:
            block = getattr(self, f'v{res}')
            xv, frames = block(xv, frames, **block_kwargs)
        
        cmap = None
        if (self.c_dim + self.l_dim) * 2 > 0:
            cmap = self.mapping_v(None, torch.cat((c, l), 1))
        xv = self.v4(xv, frames, cmap)
        
        return x, xv

    def extra_repr(self):
        return f'c_dim={self.c_dim:d}, l_dim={self.l_dim:d}, img_resolution={self.img_resolution:d}, img_channels={self.img_channels:d}'
