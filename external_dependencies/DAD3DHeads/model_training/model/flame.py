...
class FLAMELayer(nn.Module):
    ...
    def forward(self, flame_params: FlameParams, zero_rot: bool = False, zero_jaw: bool = False) -> torch.Tensor:
        ...
        return:
            vertices: B X V X 3
        """
        device = flame_params.shape.device
        bs = flame_params.shape.shape[0]
        betas = torch.cat(
            [
                flame_params.shape,
                self.shape_betas[[0]].expand(bs, -1).to(device),
                flame_params.expression,
                self.expression_betas[[0]].expand(bs, -1).to(device),
            ],
            dim=1,
        )
        neck_pose = flame_params.neck if not (0 in flame_params.neck.shape) else self.neck_pose[[0]].expand(bs, -1).to(device)
        eyeballs = flame_params.eyeballs if not (0 in flame_params.eyeballs.shape) else self.eyeballs[[0]].expand(bs, -1).to(device)
        jaw = flame_params.jaw if not (0 in flame_params.jaw.shape) else self.jaw[[0]].expand(bs, -1).to(device)

        rotation = torch.zeros([bs, ROT_COEFFS], device=device)
        if zero_jaw:
            jaw = torch.zeros_like(jaw)
        full_pose = torch.cat([rotation, neck_pose, jaw, eyeballs], dim=1)

        template_vertices = self.v_template.unsqueeze(0).repeat(bs, 1, 1).to(device)
