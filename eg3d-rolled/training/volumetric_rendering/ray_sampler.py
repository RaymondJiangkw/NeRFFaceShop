...
class RaySampler(torch.nn.Module):
    ...
    def forward(self, cam2world_matrix, intrinsics, resolution, projection='perspective'):
        """
        ...
        if projection == 'orthographic':
            ray_dirs = torch.zeros((N, M, 3), device=cam2world_matrix.device)
            ray_dirs[:, :, -1] = 1
            ...
            fy = intrinsics[:, 1, 1]
            fz = intrinsics[:, 2, 2]
            trans_x = intrinsics[:, 0, -1]
            trans_y = intrinsics[:, 1, -1]
            trans_z = intrinsics[:, 2, -1]
            
            x_cam = uv[:, :, 0].view(N, -1)
            y_cam = uv[:, :, 1].view(N, -1)
            z_cam = torch.zeros((N, M), device=cam2world_matrix.device)
            
            x_lift = (x_cam - 2 * trans_x.unsqueeze(-1)) / fx.unsqueeze(-1)
            y_lift = (y_cam - 2 * trans_y.unsqueeze(-1)) / fy.unsqueeze(-1)
            z_lift = (z_cam + 2 * trans_z.unsqueeze(-1)) / fz.unsqueeze(-1)
